...
class RegistriesPublicService {
  ...
  sendObject(data: any) {
    return publicClient.post<any>('object', {
  ...
  sendMarketPlace(data: any) {
    return publicClient.post<any>('marketplace', [
      {
  ...
  createTypePriceList(data: any) {
    return publicClient.post<any>('pricing', {
  ...
  createPriceList(data: any) {
    return publicClient.post<any>('pricinglist', [
      {
        description: 'nzm',
        priority: 1,
  ...
  sendPointOfSales(data: any) {
    return publicClient.post<any>('pointofsale', [
      {
  ...
  sendCompanies(data: any) {
    let today = new Date();
    return publicClient.post<any>('company', {
  ...
  sendDistributor(data: any) {
    ...
    return publicClient.post<any>('distributor', {
  ...
  sendDistributorCompany(
    ...
    idDistributor?: number | string
  ) {
    return publicClient.put<any>(`distributor/${idCompany}/${idDistributor}`);
  }

  sendsubscribe(data: any) {
    return publicClient.post<any>('subscribe', {
      apiKey: data.data.apiKey,
  ...
  sendsubscribeUpdate(data: any) {
    return publicClient.put<any>(`subscribe/${data.data.idCompany}`, {
      apiKey: data.data.apiKey,
      companyId: data.data.idCompany,
      companyName: data.data.companyName,
    });
  ...
  sendWarehouse(data: any) {
    return publicClient.post<any>('warehouse', [
      {
  ...
  sendUnit(data: any) {
    return publicClient.post<any>('unit', [
      {
  ...
  sendGroup(data: any) {
    return publicClient.post<any>('group', [
      {
  ...
  sendVat(data: any) {
    return publicClient.post<any>('vat', [
      {
        name: data.data.name,
  ...
  sendUsers(data: any) {
    return publicClient.post<any>('/user/registration', {
      username: data.data.username,
      password: data.data.password,
      roleName: [data.data.userRole.name],
      companies: [data.data.companyId],
    });
  }

  getObjects(id: number | string) {
    return publicClient.get<any>(`object/${id}`);
  }

  getMarketPlacec(id: number | string) {
    return publicClient.get<any>(`marketplace/company/${id}`);
  }

  getPointOfSales(id: number | string) {
    return publicClient.get<any>(`pointofsale/company/${id}`);
  }

  getCompanies() {
    return publicClient.get<any>('company');
  }

  getCompaniesDistributor(idCompany: number | string) {
    return publicClient.get<any>(`distributor/company/${idCompany}`);
  }

  getUnits() {
    return publicClient.get<any>('unit');
  }

  getVat() {
    return publicClient.get<any>('vat');
  }

  getUsers(id: number | string) {
    return publicClient.get<any>(`user/${id}`);
  }

  getGroups(uuid: number | string) {
    return publicClient.get<any>(`group/${uuid}`);
  }

  getWarehouses(uuidMarketPlace: string) {
    return publicClient.get<any>(`warehouse//marketplace/${uuidMarketPlace}`);
  }

  getCompanyInfo(id: number | string) {
    return publicClient.get<any>(`company/company/${id}`);
  }
  ...
  getSync(apiKey: string) {
    ...
    return publicClient.post<any>(`invoice/sync`, '', config);
  }

  updateCompanies(idCompany: string | number, data: any) {
    let today = new Date();
    return publicClient.put<any>(`company/${idCompany}`, {
  ...
  updateUser(id: number | string, data: any) {
    return publicClient.put<any>(`user/edit/${id}`, {
