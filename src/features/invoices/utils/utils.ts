...
const createMonetaryTotal = (invoice: any): any => {
  ...
  if (
    ...
    invoice?.advanceAccountList?.length
  ) {
  ...
  return {
    ...
    prepaidAmount:
      invoice?.invoice?.invoiceTypeCode === 380 &&
      invoice?.advanceAccountList?.length
        ? Number(prepaidAmountTotal.toFixed(2))
        : 0, // TODO for prepayment
    payableAmount:
      invoice?.invoice?.invoiceTypeCode === 380 &&
      invoice?.advanceAccountList?.length
        ? Number(invoice?.invoice?.finalSum.toFixed(2)) -
          Number(prepaidAmountTotal.toFixed(2))
        : invoice?.invoice?.finalSum.toFixed(2),
  };
};
  ...
  const errorsMessage = [
    {
      ...
      message: 'Proverite na sefu-u api key i da li je api status aktivan',
    },
    {
      error: 'contains duplicates',
      message:
        'Faktura koju pokušavate da pošaljete sa istim brojem već postoji na SEF.  Proverite da li si ste već poslali tu fakturu',
    },
    {
      error: 'Quanitity on invoice lines of prepayment must be 1',
      message: 'Kada saljete avans, kolicina ne sme biti veca od 1',
    },
    {
      error: 'Source invoice not exists',
      message: 'Morate uneti izvornu fakturu',
    },
    {
      error:
        'IssueDate is not correct because it cannot be different from todays',
      message: 'SEF prihvata isključivo e-Fakture sa današnjim datumom',
    },
    {
      error: 'When receiver is budget user order number',
      message:
        'Za budžetske korisnike je obavezno jedno od polja “broj narudžbine” ili “broj ugovora” ili “broj sporazuma”',
    },
    {
      error: 'Selected prepayment invoice',
      message:
        'Firma kojoj fakturišete još nije prihvatila prethodno poslatu avansnu e-Fakturu',
    },
    {
      error: 'Invalid user type for debtor',
      message:
        'Fakture ne treba da se šalju i na CRF za datog budžetskog korisnika',
    },
    {
      error: 'must be approved',
      message:
        ' Knjižno odobrenje, ne može biti poslato jer faktura po kojoj ga izdajete nije prihvaćena, niti odbijena',
    },
    {
      error: 'mandatory for CreditNote document',
      message:
        'Knjižno odobrenje koje šaljete na SEF nema referencu na fakturu za koju je urađen povrat',
    },
    {
      error: 'Company with identifier',
      message: 'Proverite da li je korisnik registrovan na SEF-u.',
    },
    { error: '503 Service Unavailable', message: 'Poreska nije dostupna' },
    { error: '504 Gateway Time-out', message: 'Poreska nije dostupna' },
    {
      error: 'VAT registration code must be 9 or 13 characters long',
      message: '',
    },
    {
      error: "The receiver's registration code does not have a good length",
      message: 'Dobavljač nema ispravan matični broj',
    },
  ];

  const errorTranslate: any = errorsMessage.filter((item) =>
    error.includes(item.error)
  );
  ...
  xml2js.parseString(xmlTmp, (err, result) => {
    ...
    objectTmp = {
      AccountingCustomerParty: {
        ...
        adress:
          result?.Invoice['cac:AccountingCustomerParty'][0]['cac:Party'][0][
            'cac:PostalAddress'
          ][0]['cbc:StreetName'][0],
        city: result?.Invoice['cac:AccountingCustomerParty'][0]['cac:Party'][0][
          'cac:PostalAddress'
        ][0]['cbc:CityName'][0],
        pib: result?.Invoice['cac:AccountingCustomerParty'][0]['cac:Party'][0][
          'cbc:EndpointID'
        ][0]['_'],
        mb: result?.Invoice['cac:AccountingCustomerParty'][0]['cac:Party'][0][
          'cac:PartyLegalEntity'
        ][0]['cbc:CompanyID'][0],
      },
      AccountingSupplierParty: {
        name: result?.Invoice['cac:AccountingSupplierParty'][0]['cac:Party'][0][
          'cac:PartyLegalEntity'
        ][0]['cbc:RegistrationName'][0],
        adress:
          result?.Invoice['cac:AccountingSupplierParty'][0]['cac:Party'][0][
            'cac:PostalAddress'
          ][0]['cbc:StreetName'][0],
        city: result?.Invoice['cac:AccountingSupplierParty'][0]['cac:Party'][0][
          'cac:PostalAddress'
        ][0]['cbc:CityName'][0],
        pib: result?.Invoice['cac:AccountingSupplierParty'][0]['cac:Party'][0][
          'cbc:EndpointID'
        ][0]['_'],
        mb: result?.Invoice['cac:AccountingSupplierParty'][0]['cac:Party'][0][
          'cac:PartyLegalEntity'
        ][0]['cbc:CompanyID'][0],
      },
      paymentMeans:
        result?.Invoice['cac:PaymentMeans'][0]['cac:PayeeFinancialAccount'][0][
          'cbc:ID'
        ][0],
      paymentMode: result?.Invoice['cac:PaymentMeans'][0]['cbc:PaymentID'][0],
      dueDate: result?.Invoice['cbc:DueDate'][0],
      delivery: result?.Invoice['cac:Delivery']
        ? result?.Invoice['cac:Delivery'][0]['cbc:ActualDeliveryDate'][0]
        : '',
      ...
      legalMonetaryTotal: {
        payableAmount:
          result?.Invoice['cac:LegalMonetaryTotal'][0]['cbc:PayableAmount'][0][
            '_'
          ],
        lineExtensionAmount:
