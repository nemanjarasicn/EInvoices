...
const filterItems = [
  ...
  {
    index: 4,
    name: 'InvoiceStatuses.approved',
    label: 'Approved',
    value: 'Approved',
  },
  {
    index: 5,
    name: 'InvoiceStatuses.rejected',
    label: 'Rejected',
    value: 'Rejected',
  },
  {
    index: 6,
    name: 'InvoiceStatuses.storno',
    label: 'Storno',
    value: 'Storno',
  },
  {
    index: 7,
    name: 'InvoiceStatuses.sending',
    label: 'Sending',
    value: 'Sending',
  },
  ...
  {
    index: 9,
    name: 'InvoiceStatuses.mistake',
    label: 'Mistake',
    value: 'Mistake',
  },
  {
    index: 10,
    name: 'InvoiceStatuses.overDue',
    label: 'OverDue',
    value: 'OverDue',
  },
  {
  ...
  {
    index: 12,
    name: 'InvoiceStatuses.deleted',
    label: 'Deleted',
    value: 'Deleted',
  },
  {
    index: 13,
    name: 'InvoiceStatuses.unknown',
    label: 'Unknown',
    value: 'Unknown',
  },
...
export default function FilterModal(props: any) {
  ...
  const onSubmit = async (data: any) => {
    ...
    const listData = Object.keys(data).filter(
      (item) => item === 'InvoiceStatuses'
    );
    const arrayData = data[listData[0]];
    const array = Object.keys(arrayData).filter(
      (item) => arrayData[item] !== undefined && arrayData[item] !== false
    );
    const invoicesStatusList = array.map((item) => arrayData[item]);
    //------------------

    //  logic for typeDocument
    const arrayTypeDocument = Object.keys(data).filter(
    ...
    >
      <Box sx={style}>
        ...
        >
          ...
          <Grid
            sx={{ display: 'flex', height: '10%', justifyContent: 'center' }}
          >
